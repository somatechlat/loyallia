...
| Vault | 🟢 Healthy | KV v2; seals on restart (manual unseal required) |
| Google Wallet | ⚠️ Placeholder | Service account and issuer have placeholder values in production Vault |
| Apple Wallet | ⚠️ Placeholder | Certificates and private keys have placeholder values in production Vault |
...
| Nginx | 🟢 Healthy | Reverse proxy active, SSL via Let's Encrypt |
| Production Server | 🟢 Healthy | `140.82.15.48` / `loyallia` — 31GB RAM, 4 CPU, 94GB disk |

**Test Credentials (Production E2E):**
- Owner: `e2e-owner@loyallia.com`
- Manager: `e2e-manager@loyallia.com`
- Staff: `e2e-staff@loyallia.com`
- SuperAdmin: `e2e-superadmin@loyallia.com`
- Password: generated by `provision_production_e2e_test_users --generate`

**Production Credentials:**
- SuperAdmin: `admin@loyallia.com` (see `docs/08-references/PRODUCTION_SERVER_STATE.md`)
- Owner: `owner@testloyallia.com` (see `docs/08-references/PRODUCTION_SERVER_STATE.md`)
...
# Run smoke tests (safe, read-only)
cd frontend && npx playwright test --project=auth,role-isolation

# Run full suite (creates + deletes test data)
...
| `docs/03-guides/PRODUCTION_E2E_TESTING.md` | Production E2E testing guide |
| `docs/04-runbooks/E2E_TESTING_RUNBOOK.md` | Step-by-step E2E testing runbook |
| `docs/08-references/PRODUCTION_SERVER_STATE.md` | Production server status and credentials |
