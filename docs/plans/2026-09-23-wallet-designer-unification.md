...
| U2 | **"Guardar" actually persists.** `WalletStudio.tsx` `handleSave` currently only calls `onSave?.(displayState); sessionRecovery.clearRecovery();`, and all three mount sites wire `onSave={(state) => setWalletDesign(state)}` — work is silently lost and the button lies. Make `handleSave` hit the API (or delete it and promote the parent Save). Single draft key. Honest "Guardado" / "Error al guardar" feedback. | Done |
| U3 | **Give the keyboard back.** `useKeyboardShortcuts.ts` must stop hijacking the browser: remove the Tab `preventDefault` + custom focus cycle; Delete/Backspace must confirm or raise an undo toast instead of destroying fields silently; unbind the bare `B` (require a modifier) so typing does not flip the pass. | Done |
