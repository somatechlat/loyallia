...
| LYL-CUR-APPLE-003 | Add NFC as an optional gated feature requiring Apple approval and real reader validation. | P0 | VERIFYING | `docs/APPLE_WALLET_WEB_PKPASS_NFC.md`, pass engine | source review | DOC PASS, CODE PENDING | NFC requires Apple special entitlement/approval and `nfc` pass dictionary. |
...
| LYL-CUR-APPLE-005 | Remove Verify with Wallet identity keys from current web Wallet readiness gate. | P0 | OPEN | readiness commands, production settings | readiness command | PENDING | Current command still requires identity keys and must be corrected. |
...
| LYL-CUR-APPLE-007 | Add pre-Docker Apple Wallet readiness check for web PKPass/NFC only. | P0 | OPEN | `backend/apps/api/management/commands/check_apple_wallet_config.py` | `docker compose exec -T api python manage.py check_apple_wallet_config` | FAIL: wrong scope + Vault sealed | Must validate PKPass cert/key/WWDR and optional NFC only. |
