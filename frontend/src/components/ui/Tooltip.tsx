...
export default function Tooltip({ text, position = 'top', size = 16 }: TooltipProps) {
  const [visible, setVisible] = useState(false);
  const [coords, setCoords] = useState<{ top: number; left: number; actualPos: string }>({ top: 0, left: 0, actualPos: position });
  ...
  const recalculate = useCallback(() => {
    ...
    switch (position) {
      case 'top':
        ...
        if (top < 8) { actualPos = 'bottom'; top = iconRect.bottom + padding; }
        break;
      case 'bottom':
        top = iconRect.bottom + padding;
        left = iconRect.left + iconRect.width / 2 - tipRect.width / 2;
        if (top + tipRect.height > window.innerHeight - 8) { actualPos = 'top'; top = iconRect.top - tipRect.height - padding; }
        break;
      case 'left':
        top = iconRect.top + iconRect.height / 2 - tipRect.height / 2;
        left = iconRect.left - tipRect.width - padding;
        if (left < 8) { actualPos = 'right'; left = iconRect.right + padding; }
        break;
      case 'right':
        top = iconRect.top + iconRect.height / 2 - tipRect.height / 2;
        left = iconRect.right + padding;
        if (left + tipRect.width > window.innerWidth - 8) { actualPos = 'left'; left = iconRect.left - tipRect.width - padding; }
        break;
    }
