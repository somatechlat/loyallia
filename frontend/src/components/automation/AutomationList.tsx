        ...
        <div key={a.id} className="card p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-surface-400 mt-0.5">
              ...
              {a.last_executed ? ` · ${t("automation.list.lastExecution", { date: new Date(a.last_executed).toLocaleDateString("es-EC") })}` : ""}
            </p>
          </div>

          <div className="flex items-center gap-2">
            {isOwner && (
              <button onClick={() => onEdit(a)} className="btn-ghost text-sm p-2" title={t("common.edit")}>
            ...
            >
              {a.is_active ? t("automation.action.deactivate") : t("automation.action.activate")}
            </button>
            {isOwner && (
              <button onClick={() => onDelete(a.id)} className="btn-ghost text-red-400 hover:text-red-600 text-sm p-2" title={t("common.delete")}>
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M3 6h18" />
