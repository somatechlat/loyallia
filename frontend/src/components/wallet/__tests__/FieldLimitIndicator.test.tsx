...
describe('FieldLimitIndicator', () => {
  ...
  it('renders label and count text', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="header" current={2} max={4} />
      </I18nProvider>
    );
  ...
  it('shows green color when under 50%', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="primary" current={0} max={1} />
      </I18nProvider>
    );
  ...
  it('shows yellow color between 50% and 80%', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="secondary" current={2} max={4} />
      </I18nProvider>
    );
    const progressbar = screen.getByRole('progressbar');
  ...
  it('shows red color when over 80%', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="auxiliary" current={4} max={4} />
      </I18nProvider>
    );
    const progressbar = screen.getByRole('progressbar');
  ...
  it('shows warning icon and red background when over limit', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="back" current={9} max={8} />
      </I18nProvider>
    );
  ...
  it('has correct aria attributes', () => {
    render(
      <I18nProvider>
        <FieldLimitIndicator group="header" current={1} max={3} />
      </I18nProvider>
    );
    const region = screen.getByRole('region');
    expect(region.getAttribute('aria-label')).toBe('Encabezado Field Limits: 1 / 3');
    const progressbar = screen.getByRole('progressbar');
