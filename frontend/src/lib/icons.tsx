...
/**
 * SVG path data for dashboard navigation icons.
 * Each entry maps a semantic icon name to its `path` `d` attribute.
 */
...
export interface NavIconProps {
  /** Semantic name of the icon (key into {@link NAV_ICONS}). */
  icon: string;
  /** Tailwind / CSS classes for sizing. */
  className?: string;
}

/**
 * Renders a dashboard navigation icon as an SVG.
 * Looks up the path data from {@link NAV_ICONS}.
 */
export const NavIcon = React.memo(function NavIcon({ icon, className = 'w-5 h-5' }: NavIconProps) {
