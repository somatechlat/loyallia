...
export default function PortalLoginPage() {
  ...
  const handleGeneratePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      toast.error(t('portal.login.validation.emailRequired'));
      return;
    }
    setLoading(true);
    try {
      const { data } = await portalApiClient.generatePassword(email.trim().toLowerCase());
      toast.success(data.message || t('portal.login.toast.passwordSent'));
      setStep('login');
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } }).response?.data?.message;
      toast.error(msg || t('portal.login.toast.sendError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !password.trim()) {
      toast.error(t('portal.login.validation.credentialsRequired'));
      return;
    }
    setLoading(true);
    try {
      ...
      if (data.access_token) {
        ...
        toast.success(data.message || t('portal.login.toast.welcome'));
        router.replace('/portal');
      }
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } }).response?.data?.message;
      toast.error(msg || t('portal.login.toast.loginError'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-600 via-brand-500 to-purple-600 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-white/70 mt-1 text-sm">{t('portal.login.subtitle')}</p>
        </div>
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-2xl p-8">
          {step === 'generate' ? (
            <form onSubmit={handleGeneratePassword} className="space-y-5">
              <div>
                <h2 className="text-lg font-bold text-surface-900 dark:text-white">{t('portal.login.generatePassword.title')}</h2>
                <p className="text-surface-500 text-sm mt-1">
                  {t('portal.login.generatePassword.description')}
                </p>
              </div>
              <div>
                <label className="label" htmlFor="email">{t('portal.login.emailLabel')}</label>
                <input
                  id="email"
                  type="email"
                  className="input"
              ...
              <button type="submit" className="btn-primary w-full justify-center py-3" disabled={loading}>
                {loading ? <span className="spinner w-4 h-4" /> : t('portal.login.generatePassword.submitButton')}
              </button>
              <p className="text-center text-sm text-surface-500">
                {t('portal.login.generatePassword.havePassword')}{' '}
                <button type="button" onClick={() => setStep('login')} className="text-brand-500 font-medium hover:underline">
                  {t('portal.login.signInLink')}
                </button>
              </p>
            </form>
          ) : (
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <h2 className="text-lg font-bold text-surface-900 dark:text-white">{t('portal.login.signIn.title')}</h2>
                <p className="text-surface-500 text-sm mt-1">
                  {t('portal.login.signIn.description')}
                </p>
              </div>
              <div>
                <label className="label" htmlFor="email">{t('portal.login.emailLabel')}</label>
              ...
              <div>
                <label className="label" htmlFor="password">{t('portal.login.passwordLabel')}</label>
                <div className="relative">
                  ...
                  <button
                    ...
                    aria-label={showPassword ? t('portal.login.aria.hidePassword') : t('portal.login.aria.showPassword')}
                  >
                    {showPassword ? (
              ...
              <button type="submit" className="btn-primary w-full justify-center py-3" disabled={loading}>
                {loading ? <span className="spinner w-4 h-4" /> : t('portal.login.signIn.submitButton')}
              </button>
              <p className="text-center text-sm text-surface-500">
                {t('portal.login.signIn.noPassword')}{' '}
                <button type="button" onClick={() => setStep('generate')} className="text-brand-500 font-medium hover:underline">
                  {t('portal.login.generatePasswordLink')}
                </button>
              </p>
            </form>
          )}
        ...
        <div className="text-center mt-6 space-y-2">
          <p className="text-[10px] text-white/50 space-x-3">
            <Link href="/legal/privacy" className="hover:text-white transition-colors">{t('portal.login.footer.privacyPolicy')}</Link>
          </p>
