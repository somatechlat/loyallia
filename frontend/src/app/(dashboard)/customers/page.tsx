...
interface Customer {
  ...
  phone: string; total_visits: number; total_spent: string;
  last_visit: string | null; is_active: boolean; created_at: string;
}
...
export default function CustomersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await customersApi.list({ limit: LIMIT, offset, search: search || undefined });
    ...
    try {
      ...
      if (data.success) {
        const parts = [
          ...
          data.skipped_duplicate > 0 ? `${data.skipped_duplicate} duplicados omitidos` : null,
          data.skipped_invalid > 0 ? `${data.skipped_invalid} filas inválidas` : null,
        ].filter(Boolean).join(' · ');
        toast.success(parts);
      ...
      const detail = err.response?.data?.detail || err.response?.data?.message || 'Error al importar archivo';
      toast.error(detail);
    } finally {
      setImporting(false);
    ...
    <div className="space-y-6">
      <div className="page-header flex justify-between items-center">
        <div>
          ...
          <p className="text-surface-500 text-sm mt-1">{total.toLocaleString()} clientes registrados</p>
        </div>
        {user?.role === 'OWNER' && (
          <button 
      ...
      {showImportModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-surface-900 p-6 rounded-2xl shadow-2xl w-full max-w-md">
            <h2 className="text-xl font-bold mb-1">Importar Base de Clientes</h2>
            <p className="text-sm text-surface-500 mb-4">Sube un archivo XLS, XLSX o CSV. El sistema lo depura automáticamente antes de importar.</p>

            {/* Required columns */}
            <div className="mb-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-red-500 mb-1">Columnas Obligatorias</p>
              <div className="flex gap-2 flex-wrap">
                {['email / correo', 'nombre / first_name'].map(col => (
                  <span key={col} className="bg-red-50 text-red-700 text-xs px-2 py-1 rounded border border-red-200 font-mono">{col}</span>
                ))}
              </div>
            </div>
            ...
            <p className="text-xs text-surface-400 mb-5">
              Emails duplicados se omiten automaticamente. Filas con email invalido o nombre vacio son descartadas y reportadas.
            </p>
      ...
      <div className="card p-4">
        <form onSubmit={handleSearch} className="flex gap-3">
          <input className="input flex-1" placeholder="Buscar por nombre, email o teléfono..."
            id="customer-search" value={search} onChange={e => setSearch(e.target.value)} />
          <button type="submit" className="btn-primary" id="search-btn">Buscar</button>
        </form>
      </div>
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>Visitas</th><th>Gasto total</th><th>Última visita</th><th>Estado</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="text-center py-12 text-surface-400">
                <div className="spinner w-6 h-6 mx-auto" /></td></tr>
            ) : customers.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-12 text-surface-400">No se encontraron clientes</td></tr>
            ) : customers.map(c => (
              <tr key={c.id}>
                ...
                <td className="text-surface-500 text-xs">
                  {c.last_visit ? new Date(c.last_visit).toLocaleDateString('es-EC') : '—'}
                </td>
      ...
      {total > LIMIT && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="btn-secondary" onClick={() => setOffset(o => Math.max(0, o - LIMIT))}
              disabled={offset === 0} id="prev-page-btn">Anterior</button>
            <button className="btn-secondary" onClick={() => setOffset(o => o + LIMIT)}
              disabled={offset + LIMIT >= total} id="next-page-btn">Siguiente</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <div className="text-center">
              ...
              <p className="text-white/70 mb-6">
                ¿Estás seguro de eliminar a <span className="font-semibold text-white">{customerToDelete?.name}</span>? 
