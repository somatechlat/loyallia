    ...
    <div className="space-y-6" id="programs-view">
      <div className="page-header">
        <div>
          ...
          <p className="text-surface-500 text-sm mt-1">{programs.length} programa(s) configurado(s)</p>
        </div>
        {user?.role === 'OWNER' && (
          <a href="/programs/new" className="btn-primary" id="new-program-btn">+ Nuevo programa</a>
        )}
      </div>
          ...
          {programs.map(p => (
            <div key={p.id} className="card-hover p-5 flex flex-col gap-4">
              ...
              <div className="border-t border-surface-100 pt-3 flex items-center justify-between">
                <div>
                  <p className="text-lg font-bold text-surface-900">{p.enrollments_count ?? 0}</p>
                ...
                <div className="flex items-center gap-2">
                  {user?.role === 'OWNER' && (
                    <div className="flex gap-1">
                      ...
                      <button 
                        ...
                        className="px-2 py-1 text-[10px] rounded-full bg-red-100 text-red-600 hover:bg-red-200 font-medium transition-colors"
                        title="Eliminar"
                      >
                        Eliminar
                      </button>
                  ...
                  <a href={`/programs/${p.id}`} className="p-2 text-brand-500 hover:bg-brand-50 rounded-full transition-colors" title="Ver detalles">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border border-red-100">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Eliminar Programa</h3>
