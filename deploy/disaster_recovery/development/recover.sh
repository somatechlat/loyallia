#!/usr/bin/env bash
# =============================================================================
# LOYALLIA DISASTER RECOVERY — Full Recovery from Rescue (Development)
# =============================================================================
# Reads rescue files from ./.agents/rescue/ and performs full stack recovery.
# Each step aborts on failure. Cleanup trap removes temp files.
# =============================================================================
...
step "Development Disaster Recovery"
...
# c. Destroy volumes (requires typing DESTROY)
step "Destroying volumes"
echo ""
warn "THIS WILL DESTROY ALL DATA IN DOCKER VOLUMES."
read -r -p "Type DESTROY to confirm permanent data destruction: " confirm
...
while [ "$WAITED" -lt "$MAX_WAIT" ]; do
    if $COMPOSE_CMD exec -T vault wget --spider --quiet --no-check-certificate https://127.0.0.1:8200/v1/sys/health?sealedcode=200\&uninitcode=200 2>/dev/null; then
        break
    fi
    sleep 1
    WAITED=$((WAITED + 1))
...
$COMPOSE_CMD cp "$INIT_JSON" vault:/vault/file/init.json

INIT_STATUS=$($COMPOSE_CMD exec -T vault wget -qO- --no-check-certificate https://127.0.0.1:8200/v1/sys/init 2>/dev/null || echo '{"initialized":false}')
if echo "$INIT_STATUS" | grep -q '"initialized":false'; then
    ...
    TEMP_INIT=$($COMPOSE_CMD exec -T -e VAULT_ADDR=https://127.0.0.1:8200 -e VAULT_SKIP_VERIFY=true vault \
    ...
    for key in $TEMP_KEYS; do
        $COMPOSE_CMD exec -T -e VAULT_ADDR=https://127.0.0.1:8200 -e VAULT_SKIP_VERIFY=true vault \
            vault operator unseal "$key" >/dev/null
    done

    $COMPOSE_CMD cp "$RAFT_SNAP" vault:/tmp/vault_raft_snapshot.snap
    $COMPOSE_CMD exec -T -e VAULT_TOKEN="$TEMP_TOKEN" -e VAULT_ADDR=https://127.0.0.1:8200 -e VAULT_SKIP_VERIFY=true vault \
    ...
    # After restore Vault is sealed with original keys; wait for API
    sleep 5
fi
...
SEAL_STATUS=$($COMPOSE_CMD exec -T vault wget -qO- --no-check-certificate https://127.0.0.1:8200/v1/sys/seal-status 2>/dev/null || echo '{"sealed":true}')
...
# h. Restore certs and runtime files
step "Restoring certs and runtime files"
...
step "Development recovery complete!"
