...
# =============================================================================
# Captures the ENTIRE running development system into a single encrypted tarball.
#
# What is captured:
...
#   • Host configs: nginx, .env, certs
#   • Runtime files from /run/loyallia-vault/ (host or vault container)
#
# Output:
#   $BACKUP_DIR/snapshot/loyallia_snapshot_$(timestamp).tar.gz.age
# =============================================================================
...
while IFS= read -r vol; do
    [ -z "$vol" ] && continue
    # Skip anonymous / build volumes if they look like build cache
    case "$vol" in
...
for cfg in "$PROJECT_ROOT/.env" "$PROJECT_ROOT/deploy/nginx.conf"; do
    if [ -f "$cfg" ]; then
        cp "$cfg" "$WORK_DIR/host_configs/" 2>/dev/null || warn "Could not copy $cfg"
    fi
done
# Copy certs README (actual certs live in host or deploy/certs)
...
if [ -d "/run/loyallia-vault" ]; then
    ...
    # Try to copy from vault container if not mounted on host
