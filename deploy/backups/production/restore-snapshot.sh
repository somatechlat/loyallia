...
#
# EXTRA CONFIRMATION REQUIRED: operator must type the domain name.
#
    ...
    for f in "$EXTRACT_DIR/host_configs"/*; do
        ...
        case "$fname" in
            ...
            nginx.conf)
                ...
                ;;
            rewards.loyallia.com.conf)
                cp "$f" "$PROJECT_ROOT/deploy/rewards.loyallia.com.conf" 2>/dev/null && log "Restored rewards.loyallia.com.conf" || warn "Could not restore vhost"
                ;;
            certs)
                if [ -d "$f" ]; then
