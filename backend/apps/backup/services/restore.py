...
from apps.backup.services.config import get_backup_settings, get_minio_config, temp_backup_dir
    ...
    try:
        ...
        if s3_key and minio_cfg["endpoint"] and minio_cfg["access_key"]:
            ...
            s3 = boto3.client(
                ...
                verify=getattr(__import__("django.conf", fromlist=["settings"]).settings, "AWS_S3_VERIFY", True),
            )
        ...
        subprocess.run(["tar", "-xzf", archive_path, "-C", extract_dir], check=True, capture_output=True)
        ...
        logger.warning("restore_from_backup: COMPLETED for backup %s  results=%s", backup_id, results)
