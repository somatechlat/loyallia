...
def _apply_segment_filter(queryset, segment_id: str):
    ...
    if extra == "last_30d":
        ...
        threshold_value = list(
            base.order_by("total_spent").values_list("total_spent", flat=True)[threshold_index : threshold_index + 1]
        )
...
def list_segments(request):
    ...
    agg = base_queryset.aggregate(
        ...
        lost_count=Count(
            ...
            & (Q(last_visit__lt=cutoff_60) | Q(last_visit__isnull=True, created_at__lt=cutoff_60)),
        ),
