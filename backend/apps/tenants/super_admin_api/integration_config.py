...
def normalize_and_validate_vault_secret(key: str, value: str) -> str:
    ...
    if key == "twilio_verify_service_sid":
        if not re.fullmatch(r"VA[a-fA-F0-9]{32}", normalized):
            raise HttpError(
                400,
                "Invalid Twilio Verify Service SID. Expected format: VAxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (34 chars)",
            )
        return normalized
    ...
    if key == "twilio_verify_default_channel":
        ...
        if normalized.lower() not in valid_channels:
            raise HttpError(
                400,
                f"Invalid Verify channel. Must be one of: {', '.join(sorted(valid_channels))}",
            )
    ...
    if key == "twilio_api_key_sid":
        if normalized and not re.fullmatch(r"SK[a-fA-F0-9]{32}", normalized):
            raise HttpError(
                400,
                "Invalid Twilio API Key SID. Expected format: SKxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (34 chars)",
            )
        return normalized
    ...
    if key == "twilio_test_account_sid":
        if normalized and not re.fullmatch(r"AC[a-fA-F0-9]{32}", normalized):
            raise HttpError(
                400,
                "Invalid Twilio Test Account SID. Expected format: ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (34 chars)",
            )
        return normalized
